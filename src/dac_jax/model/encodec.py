...
from dac_jax.nn.encodec_layers import StreamableConv1d, StreamableConvTranspose1d, StreamableLSTM
...
class SEANetResnetBlock(nn.Module):
    ...
    def __call__(self, x):
        ...
        if self.true_skip:
            ...
            shortcut = StreamableConv1d(self.dim, kernel_size=1, norm=self.norm, norm_kwargs=self.norm_params,
                                        causal=self.causal, pad_mode=self.pad_mode)
    ...
    def __call__(self, x):
        ...
        for i, ratio in enumerate(reversed(self.ratios)):
            ...
            for j in range(self.n_residual_layers):
                layers += [
                    SEANetResnetBlock(mult * self.n_filters, kernel_sizes=[self.residual_kernel_size, 1],
                                      dilations=[self.dilation_base ** j, 1],
                                      norm=block_norm, norm_params=self.norm_params,
                                      activation=self.activation, activation_params=self.activation_params,
                                      causal=self.causal, pad_mode=self.pad_mode, compress=self.compress,
        ...
        for i, ratio in enumerate(self.ratios):
            ...
            layers += [
                ...
                StreamableConvTranspose1d(mult * self.n_filters // 2,
                                          kernel_size=ratio * 2, stride=ratio,
                                          norm=block_norm, norm_kwargs=self.norm_params,
                                          causal=self.causal, trim_right_ratio=self.trim_right_ratio),
            ]
            # Add residual layers
            for j in range(self.n_residual_layers):
                layers += [
                    SEANetResnetBlock(mult * self.n_filters // 2, kernel_sizes=[self.residual_kernel_size, 1],
                                      dilations=[self.dilation_base ** j, 1],
                                      activation=self.activation, activation_params=self.activation_params,
                                      norm=block_norm, norm_params=self.norm_params, causal=self.causal,
                                      pad_mode=self.pad_mode, compress=self.compress, true_skip=self.true_skip)]
...
class EncodecModel(CompressionModel):
    ...
    def preprocess(self, x: jnp.ndarray) -> tp.Tuple[jnp.ndarray, tp.Optional[jnp.ndarray]]:
