def torch_to_linen(
    ...
    def parse_wn_conv(flax_params, from_prefix, to_i: int):
        ...
        d[f"WeightNorm_0"] = {
            f"Conv_0/kernel/scale": torch_params[f"{from_prefix}.weight_g"].squeeze(
                (1, 2)
            )
        }
    ...
    def parse_wn_convtranspose(flax_params, from_prefix, to_i: int):
        ...
        d[f"WeightNorm_0"] = {
            f"ConvTranspose_0/kernel/scale": torch_params[
                f"{from_prefix}.weight_g"
            ].squeeze((1, 2))
        }
    ...
    for i in range(num_codebooks):
        ...
        quantizer["in_proj"] = {
            "WeightNorm_0": {
                "Conv_0/kernel/scale": torch_params[
                    f"quantizer.quantizers.{i}.in_proj.weight_g"
                ].squeeze((1, 2))
            },
            "Conv_0": {
        ...
        quantizer["out_proj"] = {
            "WeightNorm_0": {
                "Conv_0/kernel/scale": torch_params[
                    f"quantizer.quantizers.{i}.out_proj.weight_g"
                ].squeeze((1, 2))
            },
            "Conv_0": {
