...
def mse_loss(
    predictions: jnp.ndarray, targets: jnp.ndarray, reduction="mean"
) -> jnp.ndarray:
...
class VectorQuantize(nn.Module):
    ...
    def __call__(
        ...
        commitment_loss = mse_loss(
            z_e, jax.lax.stop_gradient(z_q), reduction="none"
        ).mean([1, 2])
        codebook_loss = mse_loss(
            z_q, jax.lax.stop_gradient(z_e), reduction="none"
        ).mean([1, 2])
...
class ResidualVectorQuantize(nn.Module):
    ...
    def __post_init__(self) -> None:
        if isinstance(self.codebook_dim, int):
            self.codebook_dim = [self.codebook_dim for _ in range(self.num_codebooks)]
        super().__post_init__()

    def setup(self) -> None:

        self.quantizers = [
            VectorQuantize(self.input_dim, self.codebook_size, self.codebook_dim[i])
            for i in range(self.num_codebooks)
        ]

    def __call__(self, z, n_quantizers: int = None, train=True) -> QuantizedResult:
        z_q = 0
        residual = z
        ...
        if n_quantizers is None:
            n_quantizers = self.num_codebooks
        if train:
            n_quantizers = jnp.ones((z.shape[0],)) * self.num_codebooks + 1
            dropout = jax.random.randint(
                self.make_rng("rng_stream"),
                shape=(z.shape[0],),
                minval=1,
                maxval=self.num_codebooks + 1,
            )
        ...
        for i, quantizer in enumerate(self.quantizers):
            ...
            mask = jnp.full((z.shape[0],), fill_value=i) < n_quantizers
        ...
        codes = jnp.stack(codebook_indices, axis=1)
        latents = jnp.concatenate(latents, axis=2).transpose(0, 2, 1)
    ...
    def from_codes(self, codes: jnp.ndarray):
        ...
        return z_q, jnp.concatenate(z_p, axis=1), codes
    ...
    def from_latents(self, latents: jnp.ndarray):
        ...
        num_codebooks = jnp.where(dims <= latents.shape[2])[0].max(
        ...
        return z_q, jnp.concatenate(z_p, axis=1), jnp.stack(codes, axis=1)
...
if __name__ == "__main__":
    ...
    z_q, codes, latents, commitment_loss, codebook_loss = rvq.apply(
        {"params": params}, x, rngs={"rng_stream": jax.random.key(4)}
    )
    print(latents.shape)
